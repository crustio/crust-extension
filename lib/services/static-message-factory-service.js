...
export const getTransactionAmount = ({ metadata: { transferAmount } }) => {
  if (transferAmount === '-') {
    return 'Signed a transaction';
  }
  return `Sent ${transferAmount}`;
};
...
export const getTransfersWithMoment = transactions => {
  const modifiedTransactions = transactions.map(txn => {
    const date2 = moment(new Date(txn.block_timestamp * 1000));
    let newModifiedTime;
    ...
    if (seconds >= 1) {
      if (seconds === 1) {
        ...
        newModifiedTime = moment(new Date(txn.block_timestamp * 1000)).format('MMMM DD,YYYY');
      }
    } else {
...
export const copyAccountMessage = () => 'Address copied.';

export const copySeedPhraseMessage = () => 'Seed Phrase copied.';
