...
export const getAddressByAddr = () => {
  throw new Error('Not support in wnd wallet');
};
...
export const valueFormatter = value => {
  try {
    formatBalance.setDefaults({ unit: 'WND' });
...
export const getBalance = async address => {
  ...
  try {
    ...
    const WNDBalance = formatBalance(balance, { forceUnit: 'WND', withSi: true }, 12);
    const balanceObj = {
      address,
      balance: balance.toString(),
      amount: WNDBalance.replace(' WND', ''),
      marketData,
      balanceFormatted,
      status: SUCCESS,
    };
