...
test('Get Network for valid url', async () => {
  const url = 'getcrust.dev';
  const result = createFullNetworkURL(url);
  const network = {
    networkURL: 'ws://getcrust.dev/',
    networkPort: '9944',
    networkFullUrl: 'ws://getcrust.dev:9944/',
  };
  expect(result).toMatchObject(network);
});

test('Get Network for valid url', async () => {
  const url = 'getcrust.dev:9966';
  const result = createFullNetworkURL(url);
  const network = {
    networkURL: 'ws://getcrust.dev/',
    networkPort: '9966',
    networkFullUrl: 'ws://getcrust.dev:9966/',
  };
  expect(result).toMatchObject(network);
});

test('Get Network for valid url', async () => {
  const url = 'getcrust.dev:9966/apikey/';
  const result = createFullNetworkURL(url);
  const network = {
    networkURL: 'ws://getcrust.dev/apikey/',
    networkPort: '9966',
    networkFullUrl: 'ws://getcrust.dev:9966/apikey/',
  };
  expect(result).toMatchObject(network);
});
...
test('Throw error for Invalid url', async () => {
  const url = 'http://getcrust.dev:9966/apikey/';
  expect(() => createFullNetworkURL(url)).toThrowError('Invalid URL.');
});

test('Throw error for Invalid url', async () => {
  const url = 'https://getcrust.dev:9944/apikey/';
  expect(() => createFullNetworkURL(url)).toThrowError('Invalid URL.');
});

test('Throw error for Invalid url', async () => {
  const url = 'https://getcrust.dev:9966/apikey/';
  expect(() => createFullNetworkURL(url)).toThrowError('Invalid URL.');
});
