...
export const restoreCurrentNetwork = async network => {
  const { message, status, result } = await sendMessage({
    type: MessageTypes.BG_NETWORK_FORCE_CONNECT,
    network,
  });
  throwIfNoSuccess({ message, status });
  return { result };
...
export const getChainDecimals = async () => {
  const { result, message, status } = await sendMessage({
    type: MessageTypes.BG_NETWORK_GET_DECIMALS,
  });
  throwIfNoSuccess({ message, status });
  return { result };
};
